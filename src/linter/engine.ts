...
export async function lintWithFixes(options: LintOptions): Promise<{ report: LintReport; fixes: Map<string, string> }> {
  ...
  if (fixesByFile.size === 0) return { report, fixes }

  // Generate new migration file with all fixes
  const timestamp = new Date().toISOString().replace(/[-:T.Z]/g, '').slice(0, 14)
  const fixMigrationContent = generateFixMigration(Array.from(fixesByFile.values()).flat())

  // Determine migration directory from first file
  const firstFile = Array.from(fixesByFile.keys())[0]
  const migrationDir = firstFile.split(/[/\\]/).slice(0, -1).join('/')
  const fixMigrationPath = `${migrationDir}/${timestamp}_supavisor_fixes.sql`

  fixes.set(fixMigrationPath, fixMigrationContent)

  return { report, fixes }
}

function generateFixMigration(results: LintResult[]): string {
  const header = `-- Migration generated by Supavisor
-- Generated at: ${new Date().toISOString()}
-- This migration contains fixes for linting issues found in previous migrations
--
-- ⚠️  IMPORTANT: Review these changes before applying!
-- ⚠️  Never modify applied migrations - always create new ones
...
`
  ...
  const fixes = results
    .filter(r => r.fix)
    .map(r => {
      const source = r.file.split(/[/\\]/).pop()
      return `-- Fix for: ${r.message}
